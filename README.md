...
1. **Откройте в настройках Устройства и Службы** → **Zigbee Home Automation**
2. **Найдите ваше устройство**
3. **Нажмите три точки и "Управление устройством Zigbee"**
...
### Шаг 3: Добавление устройств для управления

1. Перейдите в **Настройки** → **Устройства и службы**
2. Найдите интеграцию **IR Remote Control**
3. Нажмите шестерёнку настройки у нужного пульта. 
...
1. В настройках пульта (шестерёнку) выберите **"Добавить команду к устройству"**
2. Выберите нужное устройство
3. Введите название команды (например, "Включить", "Громче")
4. Нажмите **"Продолжить"** после прочтения инструкции. Интеграция сразу сообщит, что команда добавлена, но на самом деле пульт начал обучение и ждёт сигнала от оригинального пульта. 
5. Нажмите нужную кнопку на оригинальном пульте, направляя его на умный пульт.
6. Команда автоматически сохранится и добавится кнопкой в виртуальное устройство в течение минуты. Если через минуту кнопка не появилась - повторите этот Шаг 4, возможно стоит поднести оригинальный пульт ближе. 
...
Для медиаплеера и кондиционера создаётся специальная сущность, то есть полноценное виртуальное устройство в HA. Например, для кондиционера создаётся регулятор температуры, выбор режимов и мощности вентилятора. Нюанс: "текущая температура" в кондиционере корректно отображаться не будет, так как сенсора температуры на самом деле нет. Он показывает лишь последнее установленное вами значения (последней отправленной команды). 
...
**ВАЖНО** для корректной работы кнопок в специальной сущности необходимо их корректно называть, интеграция будет искать подходящие кнопки по названию. Кнопки можно добавить в любой момент, сущность создаётся в любом случае, однако она просто не будет реагировать без соотвествующих команд. 
